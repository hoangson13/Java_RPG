    ...
    public void keyPressed(KeyEvent e){
        ...
        for(int i = 0 ; i < handler.object.size() ; i++){
            ...
            if (temp.getID()==ID.Player){
                ...
                if ( key == KeyEvent.VK_SPACE && (stop-start)>200 && rpgSTATE == STATE.Combat){      //Ngăn bắn liên tiếp tăng time để tăng độ chậm             
                    handler.addObject(new PlayerBullet(temp.getX()+5,temp.getY(),ID.PlayerBullet,handler));
