...
public class World {
    ...
    public World(Handler handler, String path, EntityManager entitymanager) {
        ...
        loadWorld(path);
        PlayerShip playership = new PlayerShip(entitymanager, handler, spawnX, spawnY, ID.WorldEntity);
        entitymanager.addEntity(playership);
        for (int MonNumber = 0; MonNumber < numMonster; MonNumber++) {
            entitymanager.addEntity(new EnemyShip(MonIndex, MonNumber, entitymanager, handler, 0, 0, ID.WorldEntity));
        }
    }
    ...
    private void loadWorld(String path) {
        ...
        //Lấy chỉ số cho quái
        //x,y,loại quái, máu,atk,def,speed,bullet frequency,smart bullet frequency,bullet speed
        ...
        for (int y = 0; y < height; y++) {
            for (int x = 0; x < width; x++) {
                tiles[x][y] = Utils.parseInt(tokens[(x + y * width) + 5 + numMonster * 10]); //chuyển từ array 1 chiều thành 2 chiều
            }
        }
    }
